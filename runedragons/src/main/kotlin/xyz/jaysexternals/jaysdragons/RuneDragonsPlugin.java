...
import xyz.jaysexternals.utils.JaysUtils;
...
@Extension
@PluginDependency(JaysUtils.class)
@PluginDescriptor(
	name = "Jays Rune Dragons",
	description = "Jay's Rune Dragons",
	tags = {"jays","dragons","jaysplugins"},
...
public class RuneDragonsPlugin extends Plugin
{
	...
	@Provides
	RuneDragonsConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(RuneDragonsConfig.class);
	}
	@Inject
	private RuneDragonsConfig configph;
	...
	@Inject
	private JaysUtils utils;

	private boolean inFight;
	private boolean cursed;
	private Prayer prayerToClick;
	private Random r = new Random();
	public RuneDragonsPlugin(){
		inFight = false;
	}
	...
	@Inject
	RuneDragonsOverlay runeDragonsOverlay;
	Instant botTimer;
	private NPC runeDragonNPC = null;
	public RuneDragonsState state;
	private int stage = 0;
	...
	public RuneDragonsState getState()
	{
		if (timeout > 0)
		{
			return RuneDragonsState.TIMEOUT;
		}
		if(utils.isBankOpen()){
			return getBankState();
		}
	...
	private RuneDragonsState getStates() {
		...
		if (player.getWorldArea().intersectsWith(INSIDE_DRAGONS) && configph.superantifires() && client.getVarbitValue(6101) == 0){
			return RuneDragonsState.DRINK_ANTIFIRE;
		}
		...
		if (player.getWorldArea().intersectsWith(INSIDE_DRAGONS) && !loot.isEmpty() && !utils.inventoryFull() && utils.findNearestNpc(8031) != null) {
			return RuneDragonsState.LOOT_ITEMS;
		}
		if (player.getWorldArea().intersectsWith(INSIDE_DRAGONS) && utils.inventoryContains(configph.foodID()) && utils.inventoryFull() && !loot.isEmpty() && !isInPOH(client) && utils.findNearestNpc(8031) != null) {
			return RuneDragonsState.EAT_FOOD;
		}
		if (client.getVar(Varbits.QUICK_PRAYER) != 0 && isInPOH(client)) {
			return RuneDragonsState.DEACTIVATE_PRAY;
		}
		...
		if (isInPOH(client) && client.getBoostedSkillLevel(Skill.PRAYER) < client.getRealSkillLevel(Skill.PRAYER) && configph.usePOHPool()) {
			return RuneDragonsState.DRINK_POOL;
		}
		if (isInPOH(client) && !banked) {
			return RuneDragonsState.TELE_EDGE;
		}
		if (isInPOH(client) && banked) {
			return RuneDragonsState.TELE_LITHKREN;
		}
		...
		if (player.getWorldArea().intersectsWith(INSIDE_DRAGONS) && client.getLocalPlayer().getInteracting() == null && beast.getId() == 8031) {
			return RuneDragonsState.ATTACK_RETAL; //
		}
		if (player.getWorldArea().intersectsWith(INSIDE_DRAGONS) && client.getLocalPlayer().getInteracting() == null && utils.getFirstNPCWithLocalTarget() == null) {
			return RuneDragonsState.ATTACK; //
		}
	...
	private RuneDragonsState getBankState()
	{
		...
		if (!banked){
			...
			leavingcave = false;
			banked = true;
			return RuneDragonsState.DEPOSIT_ITEMS;
		}
		if (!utils.inventoryContains(8013)){
			return RuneDragonsState.WITHDRAW_HOUSE;
		}
		if (!configph.superantifires() && !utils.inventoryContains(11951)) {
			return RuneDragonsState.WITHDRAW_ANTIFIRE;
		}
...
/*		if (configph.typecb() == CombatType.MAGIC && !configph.nomagepots() &&configph.imbuedheart() && !utils.inventoryContains(20724)){
			return RuneDragonsState.WITHDRAW_MAGIC;
		}
		...
		if (configph.typecb() == CombatType.RANGED && configph.supers() && !utils.inventoryContains(24635)){
			return RuneDragonsState.WITHDRAW_RANGED;
		}
 ...
 */
		if (configph.typecb() == CombatType.MELEE && !configph.supers() && !utils.inventoryContains(9739)){
			return RuneDragonsState.WITHDRAW_COMBAT;
		}
	...
	private void onItemSpawned(ItemSpawned event) {
		...
		if (lootableItems.stream().anyMatch(itemName.toLowerCase()::contains) && item.getId() != 1751) {
			// || client.getItemDefinition(event.getItem().getId()).getName() == "Dragon bones" || client.getItemDefinition(event.getItem().getId()).getName() == "Draconic visage") {
